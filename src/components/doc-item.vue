...
export default class DocItem extends Vue {
  ...
  get hasParams(): boolean {
    return !!this.item.params?.length;
  }

  get hasReturn(): boolean {
    return !!this.item.returns?.length;
  }
  ...
  mounted() {
    if (!this.item.description) this.description = ``;
      ...
      const replaceLink = (str: string): string => {
        ...
        if (!options.kind) return replaceLink(str.replace(text, linkName));

        const exist = docsFind(null, DocumentationService.raw$.value, options);
        if (!exist.length) return replaceLink(str.replace(text, linkName));

        return str.replace(
          text,
          `<a href="#/docs/${(options.memberof || options.name)!}${
