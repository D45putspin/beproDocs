  ...
  <div class="justify-content-sm-start">
    ...
    <span v-if="!hasArray && !hasPromise">
      <router-link
        ...
        :to="{
          name: `documentation`,
          params: { name: hasLink[0], member: hasLink[1] || undefined },
        }"
        >{{ name }}</router-link
      >
    ...
    <param-type
      v-if="hasArray || hasPromise"
      :type="name"
      :is-array-type="hasArray"
      :is-promise-type="hasPromise"
    />
  </div>
</template>
...
@Component
export default class ParamType extends Vue {
  ...
  mounted(): void {
    if (this.type.search(/^Array\.</) === 0) this.hasArray = true;

    if (this.type.search(/^Promise\.</) === 0) this.hasPromise = true;

    if (this.hasArray || this.hasPromise)
      this.name = this.type
        .replace(/^(Promise|Array)\.</g, ``)
        .replace(/>$/g, ``);
